...
# https://stackoverflow.com/questions/18914191/how-can-i-convert-1-2-3-string-to-list-1-2-3
...
def _load_question_meta_csv(path):
    ...
    with open(path, "r") as csv_file:
        ...
        for row in reader:
            try:
                data[int(row[0])] = literal_eval(row[1])
            except ValueError:
                # Pass first row.
                pass
            except IndexError:
...
def subject_count(root_dir="/data"):
    "return the total number of subjects given by subject_meta.csv"
    ...
    with open(path, "r") as csv_file:
        ...
        for row in reader:
            try:
                ...
                # is_correct might not be available.
                pass
    # assume that each subject shows up in subject_meta.csv only once
    print(len(subjects))
    return len(subjects)
...
def load_train_question_meta_csv(root_dir="/data"):
    """ Load the training question metadata as a dictionary.
